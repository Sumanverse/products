...
exports.getmodeldetails = async (req, res, next) => {
    try {
        ...
        if (model.starting_price) {
            const formattedPrice = new Intl.NumberFormat('en-IN').format(model.starting_price);
            autoFaqs.push({
                question: `What is the price of the ${model.brand_name} ${model.model_name} in ${currentCountry ? currentCountry.country_name : 'Nepal'}?`,
                answer: `The starting price of the ${model.brand_name} ${model.model_name} is ${countrySymbol} ${formattedPrice}.`
            });
        }
        ...
        return res.render('publicpages/modeldetails', {
            ...
            currencySymbol,
            countryDisplay,
            ogImage: model.model_image || '/images/mainlogo.png',
