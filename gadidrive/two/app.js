...
// URL Normalizer: Redirect uppercase URLs to lowercase
app.use((req, res, next) => {
    if (req.method === 'GET' && /[A-Z]/.test(req.path)) {
        // Only redirect GET requests to avoid breaking form submissions
        // We only check the path, preserving query parameters as they are
        res.redirect(301, req.path.toLowerCase() + (req.url.indexOf('?') > -1 ? req.url.substring(req.url.indexOf('?')) : ''));
    } else {
        next();
    }
});
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">`;
        ...
        const globalStatics = ['/', '/about', '/news', '/compare', '/emicalculator', '/privacy-policy', '/terms', '/contact'];
        ...
        categories.forEach(c => {
            const countrySlug = encodeURIComponent((c.country_name || '').toLowerCase());
            const vtSlug = encodeURIComponent((c.vt_name || '').toLowerCase().replace(/\s+/g, '-'));
            const catSlug = encodeURIComponent((c.cat_name || '').toLowerCase().replace(/\s+/g, '-'));
            if (countrySlug && vtSlug && catSlug) {
        ...
        // C. Brands
        brands.forEach(b => {
            const countrySlug = encodeURIComponent((b.country_name || '').toLowerCase());
            const vtSlug = encodeURIComponent((b.vt_name || '').toLowerCase().replace(/\s+/g, '-'));
            const brandSlug = encodeURIComponent((b.brand_name || '').toLowerCase().replace(/\s+/g, '-'));
            if (countrySlug && vtSlug && brandSlug) {
        ...
        // D. Models
        models.forEach(m => {
            const countrySlug = encodeURIComponent((m.country_name || '').toLowerCase());
            const vtSlug = encodeURIComponent((m.vt_name || '').toLowerCase().replace(/\s+/g, '-'));
            const brandSlug = encodeURIComponent((m.brand_name || '').toLowerCase().replace(/\s+/g, '-'));
            const modelSlug = encodeURIComponent((m.model_name || '').toLowerCase().replace(/\s+/g, '-'));
            if (countrySlug && vtSlug && brandSlug && modelSlug) {
        ...
        // E. Specifications
        specs.forEach(s => {
            const countrySlug = encodeURIComponent((s.country_name || '').toLowerCase());
            const vtSlug = encodeURIComponent((s.vt_name || '').toLowerCase().replace(/\s+/g, '-'));
            const brandSlug = encodeURIComponent((s.brand_name || '').toLowerCase().replace(/\s+/g, '-'));
            const modelSlug = encodeURIComponent((s.model_name || '').toLowerCase().replace(/\s+/g, '-'));
            const specSlug = encodeURIComponent((s.spec_title || '').toLowerCase().replace(/\s+/g, '-'));
            if (countrySlug && vtSlug && brandSlug && modelSlug && specSlug) {
